...
arcompact_mumbojumbo(int c, struct cpuinfo_arc *info, char *buf, int len)
{
	...
	char *cpu_nm, *isa_nm = "ARCompact", *kconf_nm;
	struct bcr_fp_arcompact fpu_sp, fpu_dp;
	int atomic = 0, be, present, saved;
	...
	kconf_nm = "CONFIG_ARC_FPU_SAVE_RESTORE";
...
static int arcv2_mumbojumbo(int c, struct cpuinfo_arc *info, char *buf, int len)
{
	int n = 0;
#ifdef CONFIG_ISA_ARCV2
	const char *release, *cpu_nm, *isa_nm = "ARCv2", *kconf_nm;
	int dual_issue = 0, dual_enb = 0, mpy_opt, present, saved;
	 ...
	 *  - Accumulator pair (r58:59) to be saved/restored if present
	 */
	present = fpu.sp | fpu.dp | mpy.dsp;	/* DSP and/or FPU */
	saved =  IS_ENABLED(CONFIG_ARC_HAS_ACCL_REGS);
	kconf_nm = "CONFIG_ARC_HAS_ACCL_REGS";
...
static void arc_chk_core_config(struct cpuinfo_arc *info)
{
	...
	if (CONFIG_ARC_DCCM_SZ != info->dccm.sz)
		panic("Linux built with incorrect DCCM Size\n");
#endif

#ifdef CONFIG_ARC_HAS_ICCM
	if (CONFIG_ARC_ICCM_SZ != info->iccm.sz)
